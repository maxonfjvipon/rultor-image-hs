...
ENV PATH="/opt/ghc/9.6.7/bin:/usr/local/bin:/root/.cabal/bin:/root/.local/bin:$PATH"

# Also inject it into skel so user 'r' inherits it
RUN echo 'export PATH=/opt/ghc/9.6.7/bin:/usr/local/bin:/root/.cabal/bin:/root/.local/bin:$PATH' >> /etc/skel/.profile
